...
// LightState is a representation of settings that can be used to change the state of a LightState.
...
// SetBrightness will set the brightness level of the LightState to the specified value.
func (s *LightState) SetBrightness(b uint8) {
	s.Brightness = b
	s.mask |= maskBrightness
}

// SetSaturation will set the light color saturation of the LightState to the specified value.
func (s *LightState) SetSaturation(v uint8) {
	s.Saturation = v
}

// SetHex will set the color of the LightState to the specified hex string value. Hex strings MUST be formalized
// with at least 6 characters and may begin with a '#' symbol.
...
// SetTemperature will set the light color temperature of the LightState to the specified value.
func (s *LightState) SetTemperature(t uint16) {
	s.Temperature = t
	s.mask |= maskTemperature
}

// SetXY will set the light color of the LightState to the specified CIE 1931 XY value.
